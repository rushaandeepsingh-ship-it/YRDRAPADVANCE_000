...
*************************************************************************************
    " READ existing Order and associated Customer
    " Provide the Order-UUID for an existing Order in %tky-uuid below
    "
*************************************************************************************
    ...
    FROM VALUE #( ( %cid    = 'Customer1'
                    %param  = VALUE #( Uuid = 'F2A1AFB65F9C1FD1AA909A60A9EED7A1'  )  ) )
    RESULT DATA(lt_function)
    FAILED DATA(lt_function_failed)
    REPORTED DATA(lt_function_reported).

    IF lt_function_failed IS INITIAL.
      out->write(
        EXPORTING
            data   = lt_function
            name   = 'FUNCTION Result : Order' ).

    ELSE.

      out->write(
        EXPORTING
        data   = lt_function_failed
        name   = 'FAILED' ).

      out->write(
        EXPORTING
        data   = lt_function_reported
        name   = 'REPORTED' ).

    ENDIF.

**************************************************************************************
*    " Link existing Order and associated Customer
*    " Provide the Order-UUID for an existing Order in %tky-uuid below
*    "
**************************************************************************************
*
...
*    ENTITY YRDR_OrderTP_000
*    EXECUTE LinkAction
*    FROM VALUE #( ( %tky-Uuid   = 'BA58FD524F671FE1AA987F250D4EE5BF'
*                    %param-Uuid = 'F2A1AFB65F9C1FD1AA909A60A9EED7A1' ) )
...
*        SELECT SINGLE * FROM zyrdorder000 WHERE order_id = '1000000088'
*        INTO @DATA(ls_new_order).
*        IF sy-subrc = 0.
*          out->write(
*            EXPORTING
*         data   = ls_new_order
*         name   = 'READ Result : Linked Order' ).
*        ENDIF.
*
*      ELSE.
*
